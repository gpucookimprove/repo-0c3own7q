...
| `skill_loader.go` | **L1 skill 注入 + L2 脚本发现**：扫描 `~/.cc-switch/skills/<id>/SKILL.md`，解析 frontmatter + 正文（`LoadAllSkills`），扫描 `scripts/*` 填充 `Skill.Scripts`，按 id 过滤启用集（`FilterEnabled`），把启用 skill 正文拼成 system prompt（`BuildSystemPromptWithSkills`）。无 YAML 依赖 |
| `skill_runner.go` | **L2 skill 工具化**：`Skill.AsMCPTools` 把每个脚本包成 `skill.<id>.<script>` 工具，`SkillTools` 聚合并进 `tools`；`SkillRunner.Dispatch` 按工具名执行脚本（python/node/bash，超时 + 错误回填为文本，`CommandBuilder` 可注入）；`CombinedDispatcher` 让 skill 工具与 MCP 工具共用一个 dispatch |
...
| `skill_runner_test.go` | scripts 发现/语言识别/`AsMCPTools`/`CombinedDispatcher` 路由/脚本执行 + 错误回填的单测 |
...
### `integration/`（接入 awy_service 的可编译示例）

| 文件 | 职责 |
|------|------|
| `awy_agent.go` | `AgentService.ChatStreamWithTools`：把**轴1（MCP 工具）+ L1（skill→system 注入）+ L2（skill 脚本工具）**装配进多轮 ReAct 循环。含 `ToolSource` 接口（`*mcpconn.Manager` 即满足）、`MCPTool→agent.Tool` 的 schema JSON round-trip、`CombinedDispatcher` 路由、真实 HTTP `LLMTransport` 接缝。搬进 awy_service 时把逻辑放进 `internal/agent/service.go` 即可 |
| `awy_agent_test.go` | 用内存 `ToolSource` + 内存 SSE 流验证：MCP 工具与 skill 工具都进 tools、首条 system 含 mode+skill 正文、skill 工具调用走 `SkillRunner` 而非 MCP |
